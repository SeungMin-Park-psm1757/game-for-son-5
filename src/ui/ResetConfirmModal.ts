...
export function showResetConfirmModal(
  ...
  const title = element('h3', '', '초기화 범위 선택');
  const description = element('p', 'muted-text', '기록만 지우거나, 싱크만 지우거나, 전체를 초기화할 수 있습니다.');
  ...
  const calibrationOnly = element('button', 'secondary-button', '싱크만 초기화');
